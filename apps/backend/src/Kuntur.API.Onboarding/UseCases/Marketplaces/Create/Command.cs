...
internal record CreateCommand(
    ...
    string EmailAddress,
    string PhoneNumber,
    string Password) : ICommand<ErrorOr<CreateCommandResult>>
{
        ...
        ILogger<CreateCommand> logger) : ICommandHandler<CreateCommand, ErrorOr<CreateCommandResult>>
    {
        ...
        public async Task<ErrorOr<CreateCommandResult>> Handle(CreateCommand cmd, CancellationToken ct)
        {
            ...
            if (existsMarketplaceResult.IsError) return existsMarketplaceResult.Errors;
            ...
            if (riskValidationResult.IsError) return riskValidationResult.Errors;
            ...
            if (createAdminResult.IsError) return createAdminResult.Errors;
            ...
            if (createSubscriptionResult.IsError) return createSubscriptionResult.Errors;
            ...
            if (createOrganizationResult.IsError) return createOrganizationResult.Errors;
            ...
            if (addMemberToOrganizationResult.IsError) return addMemberToOrganizationResult.Errors;
            ...
            if (createMarketplaceResult.IsError) return createMarketplaceResult.Errors;

            ApplicationDiagnostics.OnboardingSucceededCounter.Add(1,
                new KeyValuePair<string, object?>("marketplaceId",
                    createMarketplaceResult.Value.MarketplaceId.ToString()));
