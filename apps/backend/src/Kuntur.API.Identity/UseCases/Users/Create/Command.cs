...
internal class CreateUserCommandHandler(IIdentityProvider provider)
    : ICommandHandler<CreateUserCommand, ErrorOr<CreateUserResponse>>
{
    ...
    public async Task<ErrorOr<CreateUserResponse>> Handle(CreateUserCommand cmd, CancellationToken ct)
    {
        var result = await _provider.CreateUserAsync(
            ...
            new EmailAddress(cmd.EmailAddress),
            Password.FromPlainText(cmd.Password),
            ct);
